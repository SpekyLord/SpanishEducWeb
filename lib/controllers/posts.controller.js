...
export async function getReactions(req, res) {
  try {
    const post = await Post.findOne({
      ...
      .populate('reactions.like', 'username displayName avatar')
      .populate('reactions.love', 'username displayName avatar')
      .populate('reactions.celebrate', 'username displayName avatar')
      .populate('reactions.insightful', 'username displayName avatar')
      .populate('reactions.question', 'username displayName avatar')
    ...
    // Flatten avatar.url → avatarUrl for frontend compatibility
    ...
    res.json({
      ...
      data: {
        reactions: {
          like: mapUsers(post.reactions.like),
          love: mapUsers(post.reactions.love),
          celebrate: mapUsers(post.reactions.celebrate),
          insightful: mapUsers(post.reactions.insightful),
          question: mapUsers(post.reactions.question)
        },
        reactionsCount: post.reactionsCount
      }
    })
